    ...
    fs.readFile(
      ...
      function (err, data) {
        ...
        process.stdout.write(
          ethers.utils.defaultAbiCoder.encode(["bytes"], [artifact.bytecode])
        );
      }
    );
  }
...
function yulLogInit() {
 ...
 && rm -rf Truffle-Yulp-Box",
  ...
  exec("yul-log truffle", function (error, stdout, stderr) {
    if (error !== null) {
      console.log("exec error: " + error);
    }

    console.log(
      'Yul-Log is all set up! You can run "truffle test" to see the example tests in action.'
    );
  });
}

//function to make exec promise
...
async function yulLog() {
  ...
  if (process.argv.indexOf("hardhat") > -1) {
    ...
    framework = "truffle";
    await execPromise("truffle compile");
  }
    ...
    files.forEach((file) => {
      fs.readFile("./Yul+ Contracts/" + file, "utf8", function (err, data) {
        ...
        let output = JSON.parse(
          solc.compile(
            JSON.stringify({
              language: "Yul",
              sources: {
                "Target.yul": { content: yulp.print(source.results) },
              },
              settings: {
                outputSelection: { "*": { "*": ["*"], "": ["*"] } },
                optimizer: {
        ...
        let deployedBytecode =
          "0x" +
          output.contracts["Target.yul"][contractObjects[0]]["evm"][
            "deployedBytecode"
          ]["object"];
        if (framework == "hardhat") {
          ...
          const data = JSON.stringify(hardhatCompiled, null, 4);
          const dirname = "./artifacts/" + "contracts/" + file;
          fs.mkdirSync(dirname, { recursive: true }, (err) => {
            if (err) throw err;
          });
          fs.writeFileSync(dirname + "/" + filename[0] + ".json", data);
          console.log(`Compiled ${file} successfully\n`);
        } else if (framework == "truffle") {
          // Write compiled Yul to a Contract
          fs.writeFile(
            "./contracts/" + filename[0] + ".yul",
            yulp.print(source.results),
            (err) => {
          ...
          for (let i = 0; i < abi.length; i++) {
            if (
              abi[i].indexOf("returns") < -1 &&
              abi[i].substring(0, 5) != "event"
            ) {
              ...
              if (abi[i].substring(0, 5) != "event") {
                ...
                let funcName =
                  String(abi[i]).substring(0, closeIndex + 1) +
                  " external " +
                  String(abi[i]).substring(
                    closeIndex + 1,
                    String(abi[i]).length
                  );
                abiBlock += funcName + ";" + "\n";
          ...
          let compiledInterface = JSON.parse(
            solc.compile(
              JSON.stringify({
                language: "Solidity",
          ...
          let compiledABI =
            compiledInterface.contracts["Target.sol"]["YulpInterface"]["abi"];
          // Next read the created .json file, and inject abi into it
          const data = JSON.parse(
            fs.readFileSync("./build/contracts/" + filename[0] + ".json")
          );
          data.abi = compiledABI;
          data.deployedBytecode = deployedBytecode;
          fs.writeFileSync(
            "./build/contracts/" + filename[0] + ".json",
          ...
          fs.writeFileSync(
            "./build/" + filename[0] + ".json",
            JSON.stringify(defaultYulplusArtifact, null, 4)
          );
        }
      });
    });
